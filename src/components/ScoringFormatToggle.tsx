...
interface ScoringFormatToggleProps {
  ...
  /** Tool-page variant — a rounded pill segmented control with a volt active fill. */
  editorial?: boolean;
}
...
export function ScoringFormatToggle({ value, onChange, editorial = false }: ScoringFormatToggleProps) {
  if (editorial) {
    return (
      <div className="inline-flex gap-0.5 rounded-full bg-surface-sunken p-[3px] text-[11px]">
        {OPTIONS.map((option) => (
          <button
            key={option.value}
            type="button"
            onClick={() => onChange(option.value)}
            className={`whitespace-nowrap rounded-full px-3.5 py-1.5 uppercase tracking-[0.08em] transition-colors ${
              value === option.value ? "bg-accent font-semibold text-accent-ink" : "text-foreground/55 hover:text-foreground"
            }`}
          >
            {option.label}
          </button>
