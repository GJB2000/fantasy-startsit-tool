...
export function NewsletterSignup() {
  ...
  return (
    <div className="mb-8 flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-accent/25 bg-accent/[0.07] p-4 shadow-sm sm:p-5">
      <div className="flex items-center gap-3">
        <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-accent text-accent-ink">
        ...
        <div>
          <p className="font-display text-[17px] font-semibold leading-tight">The Legitfootball newsletter</p>
        ...
        <form onSubmit={onSubmit} className="flex flex-wrap items-center gap-2" noValidate>
          <div className="flex flex-col">
            <input
              ...
              className="w-[220px] max-w-full rounded-xl border border-foreground/15 bg-surface px-3.5 py-2.5 text-[13px] outline-none transition focus:border-accent focus:ring-2 focus:ring-accent/25 max-[560px]:w-full"
            />
          </div>
          <button
            type="submit"
            disabled={status === "loading"}
            className="rounded-xl bg-accent px-4 py-2.5 text-[13px] font-bold text-accent-ink transition hover:brightness-105 disabled:opacity-60"
          >
